...
replacement=needle+"\ntests=tests.replace(\"0.11.0-finance-v2-part1\",APP_VERSION)"
if needle not in text:
    raise RuntimeError('report fix bootstrap line not found')
script.write_text(text.replace(needle,replacement,1))
subprocess.run(['python3',str(script)],check=True)
for path in ['scripts/finance-v2-report-fix-retry.py','.github/workflows/finance-v2-report-fix-retry.yml']:
    p=Path(path)
    if p.exists(): p.unlink()
print('Report fix retry applied')
