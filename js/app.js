...
function setTab(tab) {
  ...
  if (tabChanged && tab !== 'food') state.foodAi?.controller?.abort();
  state.activeTab = tab;
...
function foodIngredientsText(value) { return Array.isArray(value) ? value.filter(Boolean).join('\n') : String(value || ''); }
function foodAmount(result) { const grams = foodNumber(result.portionGrams); const ml = foodNumber(result.volumeMl); return [grams > 0 ? `${grams} г` : '', ml > 0 ? `${ml} мл` : ''].filter(Boolean).join(' · '); }
...
function renderFoodAiCard() {
  ...
  if (ai.status === 'choosing' || ai.status === 'selecting') return `<section class="card" data-food-ai-panel role="region" aria-labelledby="food-ai-title" aria-live="polite"><div class="card-title-row"><div><h2 id="food-ai-title">Добавить блюдо по фото</h2><p class="muted">Выбери источник фотографии.</p></div></div><div class="actions"><button class="primary-button" type="button" data-food-ai-camera>Камера</button><button class="ghost-button" type="button" data-food-ai-gallery>Галерея</button><button class="ghost-button" type="button" data-food-ai-cancel>Отмена</button></div><input class="visually-hidden" data-food-ai-input="camera" type="file" accept="image/jpeg,image/png,image/webp" capture="environment"><input class="visually-hidden" data-food-ai-input="gallery" type="file" accept="image/jpeg,image/png,image/webp"></section>`;
  if (ai.status === 'analysing') return `<section class="card" data-food-ai-panel role="region" aria-labelledby="food-ai-title" aria-live="polite" aria-busy="true"><div class="card-title-row"><div><h2 id="food-ai-title">Анализирую фото…</h2><p class="muted" role="status">Это займёт немного времени.</p></div></div><button class="ghost-button" type="button" data-food-ai-cancel>Отменить</button></section>`;
  if (ai.status === 'error') return `<section class="card warning-card" data-food-ai-panel role="alert" aria-labelledby="food-ai-title"><div class="card-title-row"><div><h2 id="food-ai-title">Не удалось распознать блюдо</h2><p>${escapeHTML(ai.error || 'Попробуй ещё раз.')}</p></div></div><div class="actions"><button class="ghost-button" type="button" data-food-ai-scan aria-label="Повторить сканирование блюда">Повторить сканирование</button>${['NO_KEY','AUTH','UNSUPPORTED_PROVIDER'].includes(ai.errorCode)?'<button class="ghost-button" type="button" data-food-ai-settings>Открыть настройки</button>':''}</div></section>`;
  if (ai.status === 'result' || ai.status === 'saved') {
    const result = foodAiDraft(ai.result || {}); result.confidence = foodConfidence(result.confidence);
    return `<section class="card" data-food-ai-panel role="region" aria-labelledby="food-ai-title" aria-live="polite"><div class="card-title-row"><div><h2 id="food-ai-title">${ai.status === 'saved' ? 'Приём пищи сохранён' : 'Проверь результат'}</h2><p class="muted">Уверенность: ${escapeHTML(result.confidence ?? '—')}</p></div></div><div class="food-ai-result"><label>Название<input data-food-ai-field="name" value="${escapeHTML(result.name || '')}"></label><label>Ингредиенты<textarea data-food-ai-field="ingredients">${escapeHTML(result.ingredients)}</textarea></label><label>Комментарий<textarea data-food-ai-field="comment">${escapeHTML(result.comment || '')}</textarea></label><label>Масса, г<input inputmode="decimal" data-food-ai-field="portionGrams" value="${escapeHTML(result.portionGrams)}"></label><label>Объём, мл<input inputmode="decimal" data-food-ai-field="volumeMl" value="${escapeHTML(result.volumeMl)}"></label><div class="grid-3">${[['calories','Калории, ккал'],['protein','Белки, г'],['fat','Жиры, г'],['carbs','Углеводы, г']].map(([key,label])=>`<label>${label}<input inputmode="decimal" data-food-ai-field="${key}" value="${escapeHTML(result[key] ?? '')}"></label>`).join('')}</div></div><div class="actions"><button class="primary-button" type="button" data-food-ai-save ${ai.status === 'saved' ? 'disabled' : ''}>Подтвердить и сохранить</button><button class="ghost-button" type="button" data-food-ai-scan>Сканировать ещё</button></div></section>`;
  }
...
function renderFood() {
  const root = $('#tab-food');
  if (!root) return;
  // Stale async completions are ignored after a date change or rerender:
  // setTimeout(() => { if (state.foodAi !== ai || state.foodAi.date !== state.selectedDate) return; ai.status = 'result'; });
  const health = getHealth();
  ...
  root.innerHTML = `<section class="card" aria-labelledby="food-summary-title"><div class="card-title-row"><div><h1 id="food-summary-title">Питание</h1><p class="muted">Главное за день — энергия и быстрый ввод.</p></div></div><div class="small-stat" aria-label="Калории за выбранный день">${Math.round(nutrition.calories)}${nutrition.calories ? ' <span class="muted">ккал</span>' : ' / 2300 <span class="muted">ккал</span>'}</div>${renderFoodMacros(nutrition)}</section>
    ...
    `;
  bindCommonActions(root);
  $('[data-food-ai-scan]', root)?.addEventListener('click', () => { getFoodAiState().status = 'choosing'; renderFood(); });
  $('[data-food-ai-camera]', root)?.addEventListener('click', () => $('[data-food-ai-input="camera"]', root)?.click());
  $('[data-food-ai-gallery]', root)?.addEventListener('click', () => $('[data-food-ai-input="gallery"]', root)?.click());
  $('[data-food-ai-cancel]', root)?.addEventListener('click', () => { const ai=getFoodAiState(); ai.controller?.abort(); state.foodAi={date:state.selectedDate,status:'idle',result:null,error:''}; renderFood(); });
...
try { const provider=window.TSBApiKeyVault ? await TSBApiKeyVault.getPreference('selectedProvider') || 'gemini' : 'gemini'; const selectedModel=window.TSBApiKeyVault ? await TSBApiKeyVault.getPreference('selectedModel') || '' : ''; const model=foodGeminiModel(selectedModel); if(provider!=='gemini') throw new Error('UNSUPPORTED_PROVIDER'); if(!window.TSBApiKeyVault) throw new Error('NO_KEY'); if(!await TSBApiKeyVault.hasKey('gemini')) throw new Error('NO_KEY'); if(!window.TSBFoodAIClient || typeof window.TSBFoodAIClient.analyzeFoodPhoto!=='function') throw new Error('API'); const key=await TSBApiKeyVault.readKey('gemini'); const result=await window.TSBFoodAIClient.analyzeFoodPhoto({file,apiKey:key,model:foodGeminiModel(model),signal:ai.controller.signal}); if(state.foodAi!==ai||ai.requestId!==requestId||ai.date!==state.selectedDate)return; ai.result=result; ai.status='result'; renderFood(); }
    catch(error) { if(error?.name==='AbortError'||error?.message==='ABORTED'||state.foodAi!==ai||ai.requestId!==requestId)return; ai.errorCode=error?.code||error?.message||'API'; ai.error=({UNSUPPORTED_PROVIDER:'Выбранный провайдер пока не поддерживается.',NO_KEY:'Добавь Gemini-ключ в настройках.',AUTH:'Проверь Gemini-ключ в настройках.',QUOTA:'Лимит Gemini исчерпан.',NETWORK:'Нет связи с сервисом.',TIMEOUT:'Анализ занял слишком много времени.',INVALID_RESPONSE:'Сервис вернул непонятный результат.',INVALID_IMAGE:'Выбери JPEG, PNG или WebP.',IMAGE_TOO_LARGE:'Фото слишком большое.',API:'Сервис анализа сейчас недоступен.',ABORTED:'Анализ отменён.'}[ai.errorCode]||'Не удалось проанализировать фото.'); ai.status='error'; renderFood(); }
    finally { ai.controller=null; input.value=''; }
  }));
  $$('[data-food-ai-field]', root).forEach(field=>field.addEventListener('input',()=>{ const ai=getFoodAiState(); if(ai.result) ai.result[field.dataset.foodAiField]=field.value; }));
  $('[data-food-ai-save]', root)?.addEventListener('click', () => { const ai=getFoodAiState(); if(!ai.result||ai.status!=='result')return; const r=foodAiDraft(ai.result); const values=['calories','protein','fat','carbs','portionGrams','volumeMl']; if(!String(r.name||'').trim()||values.some(k=>!Number.isFinite(foodStrictNumber(r[k], ['portionGrams','volumeMl'].includes(k)))||foodStrictNumber(r[k], ['portionGrams','volumeMl'].includes(k))<0)||foodStrictNumber(r.calories)>20000||foodStrictNumber(r.protein)>5000||foodStrictNumber(r.fat)>5000||foodStrictNumber(r.carbs)>5000||foodStrictNumber(r.portionGrams,true)>20000||foodStrictNumber(r.volumeMl,true)>20000||String(r.name).length>160||String(r.ingredients).length>3600||String(r.comment||'').length>1000){showToast('Проверь название и значения блюда');return;} const amount=foodAmount(r); const comment=[String(r.ingredients||'').trim(),String(r.comment||'').trim()].filter(Boolean).join('\n\n'); getHealth().meals.push({id:uid('meal'),type:'Приём пищи',name:String(r.name).trim(),amount,time:'',comment,calories:foodNumber(r.calories),protein:foodNumber(r.protein),fat:foodNumber(r.fat),carbs:foodNumber(r.carbs),createdAt:new Date().toISOString()}); saveData(app,true); ai.status='saved'; renderFood(); showToast('Блюдо сохранено'); });
