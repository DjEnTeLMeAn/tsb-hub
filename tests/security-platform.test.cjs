...
const expectedCsp = "default-src 'self'; script-src 'self'; connect-src 'self' https://generativelanguage.googleapis.com https://api.openai.com; img-src 'self' data: blob:; style-src 'self' 'unsafe-inline'; object-src 'none'; base-uri 'none'; form-action 'self'; frame-src 'none'; worker-src 'self'; manifest-src 'self'";
...
test('service worker uses the version.json release for cache identity and shell URLs', () => {
  ...
  assert.ok(sw.includes('`./js/openai-food-ai-client.js?v=${RELEASE}`'));
});
