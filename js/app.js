...
function renderFood() {
  ...
  $('[data-food-ai-demo]', root)?.addEventListener('click', () => { const ai = getFoodAiState(); ai.status = 'analysing'; renderFood(); setTimeout(() => { if (state.foodAi !== ai || state.foodAi.date !== state.selectedDate) return; ai.status = 'result'; ai.result = { name: 'Демо-блюдо', calories: 520, protein: 28, fat: 19, carbs: 54 }; renderFood(); }, 450); });
