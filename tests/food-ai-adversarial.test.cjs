...
test('UI provider selection, exact Qwen model filtering, and no file/key persistence',()=>{assert.match(app,/\['gemini','qwen'\]/);assert.match(app,/foodGeminiModel/);assert.match(app,/foodQwenModel/);assert.match(app,/model === 'qwen3\.8-flash'/);assert.match(app,/qwen3\.8-flash/);assert.doesNotMatch(app,/qwen3\.8-flash-evil/);assert.doesNotMatch(app,/qwen-vl/);assert.match(app,/input\.value=''/);assert.match(app,/requestId/);assert.match(app,/ai\.requestId!==requestId/);assert.doesNotMatch(app,/localStorage[\s\S]{0,300}(?:apiKey|API key)/i)});
